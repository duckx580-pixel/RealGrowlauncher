package androidx.constraintlayout.widget;

import android.content.Context;
import android.content.res.TypedArray;
import android.util.AttributeSet;
import android.util.Log;
import android.util.SparseIntArray;
import com.rtsoft.growtopia.R;

/* JADX INFO: compiled from: r8-map-id-216c52465204c007d2cab591749625bb7374c85f4921881fbf339e9fc0348ecb */
/* JADX INFO: loaded from: classes.dex */
public final class k {

    /* JADX INFO: renamed from: p0, reason: collision with root package name */
    public static final SparseIntArray f1356p0;
    public int A;
    public float B;
    public int C;
    public int D;
    public int E;
    public int F;
    public int G;
    public int H;
    public int I;
    public int J;
    public int K;
    public int L;
    public int M;
    public int N;
    public int O;
    public int P;
    public int Q;
    public int R;
    public int S;
    public float T;
    public float U;
    public int V;
    public int W;
    public int X;
    public int Y;
    public int Z;

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    public boolean f1357a;

    /* JADX INFO: renamed from: a0, reason: collision with root package name */
    public int f1358a0;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    public int f1359b;

    /* JADX INFO: renamed from: b0, reason: collision with root package name */
    public int f1360b0;

    /* JADX INFO: renamed from: c, reason: collision with root package name */
    public int f1361c;

    /* JADX INFO: renamed from: c0, reason: collision with root package name */
    public int f1362c0;

    /* JADX INFO: renamed from: d, reason: collision with root package name */
    public int f1363d;

    /* JADX INFO: renamed from: d0, reason: collision with root package name */
    public float f1364d0;

    /* JADX INFO: renamed from: e, reason: collision with root package name */
    public int f1365e;

    /* JADX INFO: renamed from: e0, reason: collision with root package name */
    public float f1366e0;

    /* JADX INFO: renamed from: f, reason: collision with root package name */
    public float f1367f;

    /* JADX INFO: renamed from: f0, reason: collision with root package name */
    public int f1368f0;

    /* JADX INFO: renamed from: g, reason: collision with root package name */
    public boolean f1369g;

    /* JADX INFO: renamed from: g0, reason: collision with root package name */
    public int f1370g0;

    /* JADX INFO: renamed from: h, reason: collision with root package name */
    public int f1371h;

    /* JADX INFO: renamed from: h0, reason: collision with root package name */
    public int f1372h0;

    /* JADX INFO: renamed from: i, reason: collision with root package name */
    public int f1373i;

    /* JADX INFO: renamed from: i0, reason: collision with root package name */
    public int[] f1374i0;
    public int j;

    /* JADX INFO: renamed from: j0, reason: collision with root package name */
    public String f1375j0;

    /* JADX INFO: renamed from: k, reason: collision with root package name */
    public int f1376k;

    /* JADX INFO: renamed from: k0, reason: collision with root package name */
    public String f1377k0;

    /* JADX INFO: renamed from: l, reason: collision with root package name */
    public int f1378l;

    /* JADX INFO: renamed from: l0, reason: collision with root package name */
    public boolean f1379l0;

    /* JADX INFO: renamed from: m, reason: collision with root package name */
    public int f1380m;

    /* JADX INFO: renamed from: m0, reason: collision with root package name */
    public boolean f1381m0;

    /* JADX INFO: renamed from: n, reason: collision with root package name */
    public int f1382n;

    /* JADX INFO: renamed from: n0, reason: collision with root package name */
    public boolean f1383n0;

    /* JADX INFO: renamed from: o, reason: collision with root package name */
    public int f1384o;

    /* JADX INFO: renamed from: o0, reason: collision with root package name */
    public int f1385o0;

    /* JADX INFO: renamed from: p, reason: collision with root package name */
    public int f1386p;

    /* JADX INFO: renamed from: q, reason: collision with root package name */
    public int f1387q;

    /* JADX INFO: renamed from: r, reason: collision with root package name */
    public int f1388r;

    /* JADX INFO: renamed from: s, reason: collision with root package name */
    public int f1389s;

    /* JADX INFO: renamed from: t, reason: collision with root package name */
    public int f1390t;

    /* JADX INFO: renamed from: u, reason: collision with root package name */
    public int f1391u;

    /* JADX INFO: renamed from: v, reason: collision with root package name */
    public int f1392v;
    public float w;

    /* JADX INFO: renamed from: x, reason: collision with root package name */
    public float f1393x;

    /* JADX INFO: renamed from: y, reason: collision with root package name */
    public String f1394y;

    /* JADX INFO: renamed from: z, reason: collision with root package name */
    public int f1395z;

    static {
        SparseIntArray sparseIntArray = new SparseIntArray();
        f1356p0 = sparseIntArray;
        sparseIntArray.append(43, 24);
        sparseIntArray.append(44, 25);
        sparseIntArray.append(46, 28);
        sparseIntArray.append(47, 29);
        sparseIntArray.append(52, 35);
        sparseIntArray.append(51, 34);
        sparseIntArray.append(24, 4);
        sparseIntArray.append(23, 3);
        sparseIntArray.append(19, 1);
        sparseIntArray.append(61, 6);
        sparseIntArray.append(62, 7);
        sparseIntArray.append(31, 17);
        sparseIntArray.append(32, 18);
        sparseIntArray.append(33, 19);
        sparseIntArray.append(15, 90);
        sparseIntArray.append(0, 26);
        sparseIntArray.append(48, 31);
        sparseIntArray.append(49, 32);
        sparseIntArray.append(30, 10);
        sparseIntArray.append(29, 9);
        sparseIntArray.append(66, 13);
        sparseIntArray.append(69, 16);
        sparseIntArray.append(67, 14);
        sparseIntArray.append(64, 11);
        sparseIntArray.append(68, 15);
        sparseIntArray.append(65, 12);
        sparseIntArray.append(55, 38);
        sparseIntArray.append(41, 37);
        sparseIntArray.append(40, 39);
        sparseIntArray.append(54, 40);
        sparseIntArray.append(39, 20);
        sparseIntArray.append(53, 36);
        sparseIntArray.append(28, 5);
        sparseIntArray.append(42, 91);
        sparseIntArray.append(50, 91);
        sparseIntArray.append(45, 91);
        sparseIntArray.append(22, 91);
        sparseIntArray.append(18, 91);
        sparseIntArray.append(3, 23);
        sparseIntArray.append(5, 27);
        sparseIntArray.append(7, 30);
        sparseIntArray.append(8, 8);
        sparseIntArray.append(4, 33);
        sparseIntArray.append(6, 2);
        sparseIntArray.append(1, 22);
        sparseIntArray.append(2, 21);
        sparseIntArray.append(56, 41);
        sparseIntArray.append(34, 42);
        sparseIntArray.append(17, 41);
        sparseIntArray.append(16, 42);
        sparseIntArray.append(71, 76);
        sparseIntArray.append(25, 61);
        sparseIntArray.append(27, 62);
        sparseIntArray.append(26, 63);
        sparseIntArray.append(60, 69);
        sparseIntArray.append(38, 70);
        sparseIntArray.append(12, 71);
        sparseIntArray.append(10, 72);
        sparseIntArray.append(11, 73);
        sparseIntArray.append(13, 74);
        sparseIntArray.append(9, 75);
    }

    public final void a(Context context, AttributeSet attributeSet) {
        TypedArray typedArrayObtainStyledAttributes = context.obtainStyledAttributes(attributeSet, r.f1432e);
        int indexCount = typedArrayObtainStyledAttributes.getIndexCount();
        for (int i10 = 0; i10 < indexCount; i10++) {
            int index = typedArrayObtainStyledAttributes.getIndex(i10);
            SparseIntArray sparseIntArray = f1356p0;
            int i11 = sparseIntArray.get(index);
            switch (i11) {
                case 1:
                    this.f1386p = o.f(typedArrayObtainStyledAttributes, index, this.f1386p);
                    break;
                case 2:
                    this.I = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.I);
                    break;
                case 3:
                    this.f1384o = o.f(typedArrayObtainStyledAttributes, index, this.f1384o);
                    break;
                case 4:
                    this.f1382n = o.f(typedArrayObtainStyledAttributes, index, this.f1382n);
                    break;
                case 5:
                    this.f1394y = typedArrayObtainStyledAttributes.getString(index);
                    break;
                case 6:
                    this.C = typedArrayObtainStyledAttributes.getDimensionPixelOffset(index, this.C);
                    break;
                case 7:
                    this.D = typedArrayObtainStyledAttributes.getDimensionPixelOffset(index, this.D);
                    break;
                case 8:
                    this.J = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.J);
                    break;
                case 9:
                    this.f1392v = o.f(typedArrayObtainStyledAttributes, index, this.f1392v);
                    break;
                case 10:
                    this.f1391u = o.f(typedArrayObtainStyledAttributes, index, this.f1391u);
                    break;
                case 11:
                    this.P = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.P);
                    break;
                case 12:
                    this.Q = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.Q);
                    break;
                case 13:
                    this.M = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.M);
                    break;
                case 14:
                    this.O = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.O);
                    break;
                case 15:
                    this.R = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.R);
                    break;
                case 16:
                    this.N = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.N);
                    break;
                case 17:
                    this.f1363d = typedArrayObtainStyledAttributes.getDimensionPixelOffset(index, this.f1363d);
                    break;
                case 18:
                    this.f1365e = typedArrayObtainStyledAttributes.getDimensionPixelOffset(index, this.f1365e);
                    break;
                case 19:
                    this.f1367f = typedArrayObtainStyledAttributes.getFloat(index, this.f1367f);
                    break;
                case 20:
                    this.w = typedArrayObtainStyledAttributes.getFloat(index, this.w);
                    break;
                case 21:
                    this.f1361c = typedArrayObtainStyledAttributes.getLayoutDimension(index, this.f1361c);
                    break;
                case 22:
                    this.f1359b = typedArrayObtainStyledAttributes.getLayoutDimension(index, this.f1359b);
                    break;
                case 23:
                    this.F = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.F);
                    break;
                case 24:
                    this.f1371h = o.f(typedArrayObtainStyledAttributes, index, this.f1371h);
                    break;
                case 25:
                    this.f1373i = o.f(typedArrayObtainStyledAttributes, index, this.f1373i);
                    break;
                case 26:
                    this.E = typedArrayObtainStyledAttributes.getInt(index, this.E);
                    break;
                case 27:
                    this.G = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.G);
                    break;
                case 28:
                    this.j = o.f(typedArrayObtainStyledAttributes, index, this.j);
                    break;
                case R.styleable.AppCompatTheme_actionModeWebSearchDrawable /* 29 */:
                    this.f1376k = o.f(typedArrayObtainStyledAttributes, index, this.f1376k);
                    break;
                case R.styleable.AppCompatTheme_actionOverflowButtonStyle /* 30 */:
                    this.K = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.K);
                    break;
                case R.styleable.AppCompatTheme_actionOverflowMenuStyle /* 31 */:
                    this.f1389s = o.f(typedArrayObtainStyledAttributes, index, this.f1389s);
                    break;
                case R.styleable.AppCompatTheme_activityChooserViewStyle /* 32 */:
                    this.f1390t = o.f(typedArrayObtainStyledAttributes, index, this.f1390t);
                    break;
                case R.styleable.AppCompatTheme_alertDialogButtonGroupStyle /* 33 */:
                    this.H = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.H);
                    break;
                case R.styleable.AppCompatTheme_alertDialogCenterButtons /* 34 */:
                    this.f1380m = o.f(typedArrayObtainStyledAttributes, index, this.f1380m);
                    break;
                case R.styleable.AppCompatTheme_alertDialogStyle /* 35 */:
                    this.f1378l = o.f(typedArrayObtainStyledAttributes, index, this.f1378l);
                    break;
                case R.styleable.AppCompatTheme_alertDialogTheme /* 36 */:
                    this.f1393x = typedArrayObtainStyledAttributes.getFloat(index, this.f1393x);
                    break;
                case R.styleable.AppCompatTheme_autoCompleteTextViewStyle /* 37 */:
                    this.U = typedArrayObtainStyledAttributes.getFloat(index, this.U);
                    break;
                case R.styleable.AppCompatTheme_borderlessButtonStyle /* 38 */:
                    this.T = typedArrayObtainStyledAttributes.getFloat(index, this.T);
                    break;
                case R.styleable.AppCompatTheme_buttonBarButtonStyle /* 39 */:
                    this.V = typedArrayObtainStyledAttributes.getInt(index, this.V);
                    break;
                case R.styleable.AppCompatTheme_buttonBarNegativeButtonStyle /* 40 */:
                    this.W = typedArrayObtainStyledAttributes.getInt(index, this.W);
                    break;
                case R.styleable.AppCompatTheme_buttonBarNeutralButtonStyle /* 41 */:
                    o.g(this, typedArrayObtainStyledAttributes, index, 0);
                    break;
                case R.styleable.AppCompatTheme_buttonBarPositiveButtonStyle /* 42 */:
                    o.g(this, typedArrayObtainStyledAttributes, index, 1);
                    break;
                default:
                    switch (i11) {
                        case R.styleable.AppCompatTheme_dialogTheme /* 61 */:
                            this.f1395z = o.f(typedArrayObtainStyledAttributes, index, this.f1395z);
                            break;
                        case R.styleable.AppCompatTheme_dividerHorizontal /* 62 */:
                            this.A = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.A);
                            break;
                        case R.styleable.AppCompatTheme_dividerVertical /* 63 */:
                            this.B = typedArrayObtainStyledAttributes.getFloat(index, this.B);
                            break;
                        default:
                            switch (i11) {
                                case R.styleable.AppCompatTheme_homeAsUpIndicator /* 69 */:
                                    this.f1364d0 = typedArrayObtainStyledAttributes.getFloat(index, 1.0f);
                                    break;
                                case R.styleable.AppCompatTheme_imageButtonStyle /* 70 */:
                                    this.f1366e0 = typedArrayObtainStyledAttributes.getFloat(index, 1.0f);
                                    break;
                                case R.styleable.AppCompatTheme_listChoiceBackgroundIndicator /* 71 */:
                                    Log.e("ConstraintSet", "CURRENTLY UNSUPPORTED");
                                    break;
                                case R.styleable.AppCompatTheme_listDividerAlertDialog /* 72 */:
                                    this.f1368f0 = typedArrayObtainStyledAttributes.getInt(index, this.f1368f0);
                                    break;
                                case R.styleable.AppCompatTheme_listMenuViewStyle /* 73 */:
                                    this.f1370g0 = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.f1370g0);
                                    break;
                                case R.styleable.AppCompatTheme_listPopupWindowStyle /* 74 */:
                                    this.f1375j0 = typedArrayObtainStyledAttributes.getString(index);
                                    break;
                                case R.styleable.AppCompatTheme_listPreferredItemHeight /* 75 */:
                                    this.f1383n0 = typedArrayObtainStyledAttributes.getBoolean(index, this.f1383n0);
                                    break;
                                case R.styleable.AppCompatTheme_listPreferredItemHeightLarge /* 76 */:
                                    this.f1385o0 = typedArrayObtainStyledAttributes.getInt(index, this.f1385o0);
                                    break;
                                case R.styleable.AppCompatTheme_listPreferredItemHeightSmall /* 77 */:
                                    this.f1387q = o.f(typedArrayObtainStyledAttributes, index, this.f1387q);
                                    break;
                                case R.styleable.AppCompatTheme_listPreferredItemPaddingLeft /* 78 */:
                                    this.f1388r = o.f(typedArrayObtainStyledAttributes, index, this.f1388r);
                                    break;
                                case R.styleable.AppCompatTheme_listPreferredItemPaddingRight /* 79 */:
                                    this.S = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.S);
                                    break;
                                case R.styleable.AppCompatTheme_panelBackground /* 80 */:
                                    this.L = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.L);
                                    break;
                                case R.styleable.AppCompatTheme_panelMenuListTheme /* 81 */:
                                    this.X = typedArrayObtainStyledAttributes.getInt(index, this.X);
                                    break;
                                case R.styleable.AppCompatTheme_panelMenuListWidth /* 82 */:
                                    this.Y = typedArrayObtainStyledAttributes.getInt(index, this.Y);
                                    break;
                                case 83:
                                    this.f1358a0 = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.f1358a0);
                                    break;
                                case R.styleable.AppCompatTheme_popupWindowStyle /* 84 */:
                                    this.Z = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.Z);
                                    break;
                                case R.styleable.AppCompatTheme_radioButtonStyle /* 85 */:
                                    this.f1362c0 = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.f1362c0);
                                    break;
                                case R.styleable.AppCompatTheme_ratingBarStyle /* 86 */:
                                    this.f1360b0 = typedArrayObtainStyledAttributes.getDimensionPixelSize(index, this.f1360b0);
                                    break;
                                case R.styleable.AppCompatTheme_ratingBarStyleIndicator /* 87 */:
                                    this.f1379l0 = typedArrayObtainStyledAttributes.getBoolean(index, this.f1379l0);
                                    break;
                                case R.styleable.AppCompatTheme_ratingBarStyleSmall /* 88 */:
                                    this.f1381m0 = typedArrayObtainStyledAttributes.getBoolean(index, this.f1381m0);
                                    break;
                                case R.styleable.AppCompatTheme_searchViewStyle /* 89 */:
                                    this.f1377k0 = typedArrayObtainStyledAttributes.getString(index);
                                    break;
                                case R.styleable.AppCompatTheme_seekBarStyle /* 90 */:
                                    this.f1369g = typedArrayObtainStyledAttributes.getBoolean(index, this.f1369g);
                                    break;
                                case R.styleable.AppCompatTheme_selectableItemBackground /* 91 */:
                                    Log.w("ConstraintSet", "unused attribute 0x" + Integer.toHexString(index) + "   " + sparseIntArray.get(index));
                                    break;
                                default:
                                    Log.w("ConstraintSet", "Unknown attribute 0x" + Integer.toHexString(index) + "   " + sparseIntArray.get(index));
                                    break;
                            }
                            break;
                    }
                    break;
            }
        }
        typedArrayObtainStyledAttributes.recycle();
    }
}
